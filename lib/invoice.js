...
function drawPageChrome(doc, fonts, settings, pageH) {
  ...
  if (fs.existsSync(mark)) {
    try {
      doc.image(mark, 42, 36, { height: 26 });
    } catch (err) {
      doc.fillColor(COLORS.leaf).font(fonts.displaySemi).fontSize(20).text("Sāmānya", 42, 40);
    }
  } else {
    doc.fillColor(COLORS.leaf).font(fonts.displaySemi).fontSize(20).text("Sāmānya", 42, 40);
  }
  doc.fillColor(COLORS.moss).font(fonts.body).fontSize(8)
    .text("WELLNESS GIFTS", 42, 66, { characterSpacing: 1.6 });
...
function invoicePdf(order, settings) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (Number(order.discount) > 0) {
      doc.font(fonts.body).fontSize(10).fillColor(COLORS.ink)
        .text("Discount", cols.gift + 10, y);
      doc.font(fonts.display).fontSize(12)
        .text("−" + money(order.discount), cols.amt - 110, y, {
          width: 100,
          align: "right"
        });
      y += 22;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .wordmark { display: block; height: 34px; width: auto; }
  ...
  <article class="sheet">
    <div class="pad">
      ...
      <table>
        ...
        <tbody>
          ${lines}
          <tr><td colspan="3">Delivery</td><td>${order.shipping ? rupees(order.shipping) : "Complimentary"}</td></tr>
          ${Number(order.discount) > 0 ? `<tr><td colspan="3">Discount</td><td>−${rupees(order.discount)}</td></tr>` : ""}
        </tbody>
      </table>
...
function packSheet(order) {
  ...
  return [
    ...
    Number(order.shipping) ? "Delivery: ₹" + order.shipping : "Delivery: complimentary",
