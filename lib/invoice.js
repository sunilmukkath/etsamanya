...
function invoiceHtml(order, settings, origin) {
  ...
  const paid = ["paid", "packed", "shipped"].includes(order.status);
  const heading = paid
    ? (settings.gstin ? "Tax invoice" : "Invoice")
    : (order.status === "pending" ? "Proforma" : "Estimate");
  const payRef = paid
    ? " · PayU " + escapeHtml(order.mihpayid || order.txnid)
    : (order.status === "pending" ? " · awaiting PayU" : " · not a tax invoice");
...
<div class="sheet">
  ...
  <h1>${heading} ${escapeHtml(invoiceNo)}</h1>
  <p class="muted">${escapeHtml(when)}${payRef}</p>
  <div class="grid">
    <div>
