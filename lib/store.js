...
const COMPANY = {
  ...
  tradeName: "samanya",
  address: "274/12 Ellai Amman Koil Street, Saraswathi Nagar, Thiruvallur, Tamil Nadu 631211",
  sellerState: "Tamil Nadu",
  sellerStateCode: "33",
  email: "etsamanyafoods@gmail.com",
  phone: "9940184841"
};
...
function emptyDb() {
  ...
  return {
    settings: {
      legalName: process.env.LEGAL_NAME || COMPANY.legalName,
      tradeName: COMPANY.tradeName,
      address: COMPANY.address,
      sellerState: COMPANY.sellerState,
      sellerStateCode: COMPANY.sellerStateCode,
      email: process.env.NOTIFY_EMAIL || COMPANY.email,
      phone: process.env.NOTIFY_PHONE || COMPANY.phone,
      whatsapp: String(process.env.WHATSAPP_NUMBER || process.env.NOTIFY_PHONE || COMPANY.phone).replace(/\D/g, ""),
      whatsappAlert: String(process.env.WHATSAPP_TO || "").replace(/\D/g, ""),
      gstin: "",
      gstRate: 0,
      hsn: "",
      shippingFlat: Number(process.env.SHIPPING_FLAT || 99),
      shippingFreeAbove: Number(process.env.SHIPPING_FREE_ABOVE || 1999),
      notifyEmail: process.env.NOTIFY_EMAIL || COMPANY.email,
...
function applyDocSeries(settings) {
  const next = settings || {};
  let changed = false;
  ["nextInvoice", "nextEstimate"].forEach((key) => {
    const n = Number(next[key] || 0);
    if (!n || n < 101) {
      next[key] = 101;
      changed = true;
    }
...
function applyContact(settings) {
  ...
  if (!email || email === "hello@etsamanya.com") {
    next.email = COMPANY.email;
    changed = true;
  }
  const notify = String(next.notifyEmail || "").trim().toLowerCase();
  if (!notify || notify === "hello@etsamanya.com") {
    next.notifyEmail = COMPANY.email;
    changed = true;
  }
  if (!String(next.phone || "").replace(/\D/g, "")) {
    next.phone = COMPANY.phone;
    changed = true;
  }
  return changed;
}

function applyRegisteredOffice(settings) {
  const next = settings || {};
  let changed = false;
  const name = String(next.legalName || "").trim();
  if (!name || /^samanya$/i.test(name)) {
    next.legalName = COMPANY.legalName;
    changed = true;
  }
  const addr = String(next.address || "").trim();
  if (!addr || /Thenkulapakkam/i.test(addr)) {
    next.address = COMPANY.address;
    changed = true;
  }
  return changed;
}
...
function mergeDb(raw) {
  ...
  applyRegisteredOffice(settings);
  applyDocSeries(settings);
...
function expireReservations(db) {
  ...
  (db.orderIds || []).forEach((id) => {
    ...
    const deadline = order.expiresAt
      ...
      : Number(order.createdAt || 0) + maxAge;
...
async function readDb() {
  ...
  const officeStale = Boolean(raw && raw.settings && (
    /Thenkulapakkam/i.test(String(raw.settings.address || "")) ||
    /^samanya$/i.test(String(raw.settings.legalName || "").trim())
  ));
  const seriesStale = Boolean(raw && raw.settings && (
    Number(raw.settings.nextInvoice || 0) < 101 ||
    Number(raw.settings.nextEstimate || 0) < 101
  ));
  const contactStale = Boolean(raw && raw.settings && (
    /hello@etsamanya\.com/i.test(String(raw.settings.email || "")) ||
    /hello@etsamanya\.com/i.test(String(raw.settings.notifyEmail || "")) ||
    !String(raw.settings.phone || "").replace(/\D/g, "")
  ));
  if (expireReservations(db) || officeStale || seriesStale || contactStale) await writeDb(db);
  return db;
}
...
function publicSettings(db) {
  ...
  return {
    ...
    email: s.email,
    phone: s.phone,
    whatsapp: s.whatsapp || process.env.WHATSAPP_NUMBER || s.phone || "",
